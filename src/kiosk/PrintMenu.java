    ...
    public static void printOptionCheck(int menuNumber, List<Products> productsList) { // 옵션에 대한 질문 출력
        ...
        productsList.stream().filter(product -> product.getNumber() == menuNumber)
                .forEach(product -> {
                    ...
                    System.out.println("1. Single(W " + product.getPrice() +
                    ")\t\t 2. Double(W " + doublePrice + ")\n");
                    int selectedOptionNumber = menu.selectOption(product);
                    printCartCheck(menuNumber, selectedOptionNumber, selectedProduct.get(0));
        });
    }

    public static void printCartCheck(int menuNumber, int selectedOptionNumber, Products product) { // 장바구니에 추가할지 물어보는 질문 출력
        Menu menu = new Menu();
